...
class Pagination {
    ...
    private function GetLinks() {
        ...
        for ($x = $start_range; $x <= $end_range; $x++) {
            $links .= $this->page == $x ? '<li class="current-page">' . $x . '</li>' : '<a href="' . $this->BuildURL($x) . '"><li>' . $x . '</li></a>';
        }
        return $links;
    ...
    private function GetPrevious() {
        return ($this->page != 1) ? '<a href="' . $this->BuildURL($this->page-1) . '"><li>' . Language::GetText('previous') . '</li></a>' : '';
    }
    ...
    private function GetNext() {
        return ($this->page != $this->page_count) ? '<a href="' . $this->BuildURL($this->page+1) . '"><li>' . Language::GetText('next') . '</li></a>' : '';
    }
    ...
    private function GetFirst() {
        if (!$this->base) {
            $first = '<a href="' . $this->BuildURL(1) . '"><li>1</li></a>';
            $first .= '<a href="' . $this->BuildURL(2) . '"><li>2</li></a>';
            $first .= '<li>...</li>';
            return $first;
        } else {
            return '';
    ...
    private function GetLast() {
        if (!$this->end) {
            $last = '<li>...</li>';
            $last .= '<a href="' . $this->BuildURL($this->page_count-1) . '"><li>' . ($this->page_count-1) . '</li></a>';
            $last .= '<a href="' . $this->BuildURL($this->page_count) . '"><li>' . $this->page_count . '</li></a>';
            return $last;
        } else {
            return '';
        }
